...
//   try {
//     if (!req.session.user) {
//       // If you prefer public shop page, remove redirect
//       return res.redirect("/user/login");
//     }
...
//     // Filters from query string (all strings)
...
//     // Price filter applied to variants.price (find products that have at least one variant in range)
...
//     switch (sort) {
//       case "price-low":
//         // sort by variant price (first variant used); not perfect but OK for listing
...
//     // Remove products with invalid category (optional)
//     const cleanedProducts = products.filter(p => p.category);

//     // Unique colors and brands for sidebar UI
//     // We want these to reflect full result set (or total product pool). For simplicity use all active products.
